...
const pakets = [
  {
    ...
    color: 'from-green-500 to-yellow-400', border: 'border-green-300', bg: 'bg-green-50',
    highlight: false,
  },
  {
    ...
    color: 'from-green-500 to-green-400', border: 'border-green-300', bg: 'bg-green-50',
    highlight: true,
  },
  {
    ...
    color: 'from-rose-500 to-green-400', border: 'border-rose-300', bg: 'bg-rose-50',
    highlight: false,
  },
];
...
export default function ResellerPage() {
  ...
  const inputClass = (field: keyof ResellerInfo) =>
    `w-full px-4 py-3 rounded-xl border text-sm transition-all outline-none focus:ring-2 focus:ring-green-300 ${
      errors[field] ? 'border-red-300 bg-red-50' : 'border-green-200 bg-white focus:border-green-400'
    }`;
    ...
    <main className="min-h-screen" style={{ background: '#FFFFFF' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {resellers.map((r, i) => (
            <motion.div key={r.id}
              ...
              className="bg-white rounded-2xl p-5 border border-green-100 shadow-sm hover:shadow-md hover:border-green-200 transition-all"
            >
      ...
      <section id="daftar" className="py-14 px-4 sm:px-6 lg:px-8 max-w-2xl mx-auto">
        ...
        <motion.form initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          className="bg-white rounded-3xl border border-green-100 shadow-lg p-6 sm:p-8 space-y-5"
        >
          {/* Pilih Paket */}
          <div>
              ...
              {pakets.map(p => (
                <label key={p.id}
                  className={`flex items-center gap-3 p-3.5 rounded-xl border-2 cursor-pointer transition-all ${
                    form.paket === p.id ? 'border-green-400 bg-green-50' : 'border-green-100 hover:border-green-200'
                  }`}
                >
          ...
          <div>
            ...
            <textarea rows={3} placeholder={t.reseller.phPengalaman} value={form.pengalaman}
              ...
              className="w-full px-4 py-3 rounded-xl border border-green-200 bg-white text-sm transition-all outline-none focus:ring-2 focus:ring-green-300 focus:border-green-400 resize-none"
            />
          </div>
