...
// persis dengan karyaputra-admin/src/lib/db.ts (database yang sama, dua deployment Vercel
...
export function getSql() {
  if (!sql) {
    sql = postgres(process.env.DATABASE_URL!, {
      ...
      // Lihat catatan panjang di karyaputra-admin/src/lib/db.ts: `max: 1` terbukti bikin
